...
<div class="w-full md:w-4/5 mx-2 p-2">
  <div class="flex justify-between items-center w-auto h-16 mt-2">
    ...
    {:else}
      <span>
        <button id="addBoard" class="mx-1 p-2 rounded shadow bg-sage-300 dark:bg-sage-700 cursor-pointer" on:click={() => showModal("ADD")}>
          <Icon kind="square-plus"/>
        </button>
        <button id="remBoard" class="mx-1 p-2 rounded shadow bg-sage-300 dark:bg-sage-700 cursor-pointer" on:click={() => showModal("DELETE")}>
          <Icon kind="square-minus"/>
        </button>
        <button id="archive" class="mx-1 p-2 rounded shadow bg-sage-300 dark:bg-sage-700 cursor-pointer" on:click={handleBatchArchive}>
          <Icon kind="archive"/>
        </button>
        <button id="delete" class="mx-1 p-2 rounded shadow bg-sage-300 dark:bg-sage-700 cursor-pointer" on:click={handleBatchDelete}>
          <Icon kind="trash"/>
        </button>
      </span>
    {/if}
